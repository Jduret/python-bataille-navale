...
class Settings:
	...
	defaultHumainName = 'Joueur Humain'
	...
	#épaisseur du trait
	epaisseurTrait = 3
	#Taille du rond de placement bateau + placement touché
...
#permet de convertir un event en point (c+1,l+1)
...
#permet de convertir un point en event
...
# permet de convertir un point (ligne, colonne) en (nombre, lettre) pour faire "touche en A3 par exemple
	def pointToCase(point):
		(c, l) = point
		return Settings.columnToChar(c) + str(l)

# permet de convertir un point (nombre,lettre) en (ligne,colonne)
